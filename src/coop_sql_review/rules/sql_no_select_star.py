...
that then narrows columns), so the rule scopes to *production* selects — any
projection ``*`` whose select is NOT the inner query of a CTE. ``COUNT(*)``
and other function-argument stars are never flagged.
"""
...
def check(ctx: RuleContext) -> list[Finding]:
    ...
    for batch, select in ctx.parsed.find_all(exp.Select):
        # A CTE's own SELECT is intermediate by design — allow * there.
